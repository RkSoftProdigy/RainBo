...
- (void)onFailure:(id)data {
    
    [self onFailure:data responseCallback:nil];
    
}

- (void)onFailure:(id)data responseCallback:(WVJBResponseCallback)responseCallback {
    [_base sendData:data responseCallback:responseCallback handlerName:nil];
}
- (void)onSuccess:(id)data {
    
    [self onSuccess:data responseCallback:nil];
    
}

- (void)onSuccess:(id)data responseCallback:(WVJBResponseCallback)responseCallback {
    [_base sendData:data responseCallback:responseCallback handlerName:nil];
}
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    NSString *strUrl=[NSString stringWithFormat:@"%@",url];
     NSLog(@"%@--%@",url,strUrl);
    if ([strUrl isEqualToString:@"https://payu.herokuapp.com/ios_success"])
    {
        return YES;
    }
